  ...
  if (episodes.length > 0) {
    ...
    episodes.forEach(function(ep) {
      ...
      html += '<div style="background:var(--bg3);border-radius:8px;padding:10px;margin-bottom:8px;font-size:12px;">' +
        ...
        (epStatus === 'failed' ? (
          '<div style="margin-top:8px;display:flex;gap:6px;">' +
          '<button onclick="retryDramaEpisode(' + ep.id + ',' + project.id + ')" style="flex:1;padding:6px;border:none;border-radius:6px;background:rgba(255,71,87,0.15);color:#FF4757;font-size:11px;cursor:pointer;">🔄 重试</button>' +
          '<button onclick="editDramaEpisodeInfo(' + ep.id + ',' + project.id + ')" style="flex:1;padding:6px;border:none;border-radius:6px;background:rgba(255,255,255,0.08);color:var(--t1);font-size:11px;cursor:pointer;">✏️ 修改</button>' +
          '</div>'
        ) : '') +
        (epStatus === 'draft' ? (
          '<div style="margin-top:8px;display:flex;gap:6px;">' +
          '<button onclick="retryDramaEpisode(' + ep.id + ',' + project.id + ')" style="flex:1;padding:6px;border:none;border-radius:6px;background:#22c55e;color:#fff;font-size:11px;cursor:pointer;">▶ 继续生成</button>' +
          '<button onclick="editDramaEpisodeInfo(' + ep.id + ',' + project.id + ')" style="flex:1;padding:6px;border:none;border-radius:6px;background:rgba(255,255,255,0.08);color:var(--t1);font-size:11px;cursor:pointer;">✏️ 修改</button>' +
          '</div>'
        ) : '') +
        '</div>';
...
async function retryDramaEpisode(episodeId, projectId) {
  if (!confirm('确定要重新生成该集剧本吗？')) return;
  try {
    var token = localStorage.getItem('sdToken') || localStorage.getItem('sdApiKey');
    // Get episode number from cache
    var project = _findProject(projectId);
    var episode = project ? (project.episodes || []).find(function(ep) { return ep.id === episodeId; }) : null;
    var epNum = episode ? episode.episode_number : null;

    var body = epNum ? JSON.stringify({ episode_numbers: [epNum] }) : '{}';
    var res = await fetch('/api/drama/projects/' + projectId + '/generate-script', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
      body: body
    });
    var data = await res.json();
    if (res.ok) {
      alert('✅ ' + (data.message || '已重新生成'));
      closeDramaDetailModal();
      loadDramaWorks();
    } else {
      alert('重试失败：' + (data.detail || ''));
    }
  } catch (e) {
...
function editDramaEpisodeInfo(episodeId, projectId) {
  var project = _findProject(projectId);
  if (!project) return;
  var episode = (project.episodes || []).find(function(ep) { return ep.id === episodeId; });
  if (!episode) return;
  var newTitle = prompt('修改该集标题：', episode.title || '');
  if (newTitle === null) return;
  var newHook = prompt('修改钩子：', episode.hook || '');
  if (newHook === null) return;
  // For now, just update locally and proceed to regenerate
  closeDramaDetailModal();
  // Just retry with the existing episode number - the content
  // will be regenerated by the AI
  retryDramaEpisode(episodeId, projectId);
}
